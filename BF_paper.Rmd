...
The motivated maximum effect is twice the intercept. Using the heuristic described above, we set the expected effect $s$ to be half this value:
...
The following code specifies a range of values, and uses bespoke function `Bf_range` h1_range to calculate Bayes factors using each of these values as scale factor for the $H_1$ distribution:  
...
The researcher specifies a full mixed model which includes the parameter of interest; as in the Dienes calculator approach, the prior for this parameter represents $H_1$. The researcher then specifies a null model where the parameter of interest is excluded, meaning it is set to zero. The probability of the data under each model is then approximated using bridge sampling [@gronau2020]. This approach is likely to be more robust than the Dienes calculator approach since it compares entire models, rather than using a summary of the data; however, it also requires the researcher to have expertise in running fully Bayesian analyses.
...
The results are presented in Table \ref{tab:model-comparison-table}. As the table shows, the two approaches agree qualitatively in all cases, in terms of whether the evidence supports $H_0$, $H_1$, or is ambiguous. This suggests that at least for this simple case, the two approaches produce comparable results. 
...
```{r scatterplot, warning=FALSE,messages = FALSE,fig.cap="Scatter plot of the Bayes factors (log transformed) produced by the Dienes Calculator and the Model Comparison Methods. Different data points are from  different runs with different effect sizes and baselines (see Table 2). We separate the case where $H_1$ is true and $H_0$ is true. The line of identity is in black. The dotted lines shows where the threshold of substantial evidence for H1 (BF>3) and for the null (BF< $\\frac{1}{3}$) was found with each method (Dienes calculator blue, Model comparison red.)", echo=FALSE}
grid.arrange(p.H1.true,p.H0.true, ncol=2)

```


Figure \ref{fig:scatterplot} shows scatter plots comparing the Bayes Factors computed by the two approaches, separting cases where $H_1$ is true and $H_0$ is true. While the points are not quite evenly scattered around the line of identity, they fairly closely follow the line and the departures do not involve any qualitative change in what conclusion would follow from the Bayes factors, in line with the results in Table \ref{tab:model-comparison-table}.
